...
function cardshow2(plan_id, user_loc_response, sort_by, search, pagenumber) {
  ...
  $.ajax({
    ...
    contentType: "application/json",
    success: function (response) {
      //this is second call method
      products_result_two = response.products.result;
      products_pagination_two = response.products.pagination;
...
var products_result;
var products_result_two;
var products_pagination;
var products_pagination_two;
var E_stock;
// new code
...
function homepagecards() {
  ...
  if (products_result.length == 0 && products_result_two?.length == 0) {
  ...
  if (products_result.length != 0) {
    ...
    products_result_two != undefined &&
    products_result_two.length != 0
  ) {
    check_plant_not_same = true;
    renderProductCards(products_result_two);
    paginationlist(
      products_pagination_two.current,
      products_pagination_two.total,
      products_pagination_two.next,
      products_pagination_two.perPage,
      "NewIndent"
    );
  } else {
    cardBody.innerHTML += `<div class="cardimgbody">
...
$(document).on("input", ".count", function () {
  ...
  if ($.isNumeric(enteredValue)) {
    ...
    const datas = products_result.filter((obj) => obj.id === index);
    var addtocarts = JSON.parse(localStorage.getItem("cart"));
    debugger;
    if (
      addtocarts &&
      addtocarts.some((item) => item.special_indent != datas[0].special_indent)
    ) {
      Swal.fire({
        title: "Different Material Types in One Indent Cart",
        text: "As per the new approval flow, you can't add regular consumable items with other material in One Indent Cart. Please create two different indents to go forward. For any clarity, please connect with Central Store Team",
        icon: "warning",
        showCancelButton: true,
        confirmButtonText: "Yes",
        cancelButtonText: "No",
        ...
        if (result.isConfirmed) {
          ...
          if (parseInt(enteredValue) > stock) {
            ...
            stock - enteredValue >= 0
              ? val.val(stock - enteredValue)
              : val.val(0);
            add_to_cart_count();
          } else {
    ...
    function add_to_cart_count() {
      ...
      var temp = {
        ...
        special_indent: data[0].special_indent ?? "0",
      };
