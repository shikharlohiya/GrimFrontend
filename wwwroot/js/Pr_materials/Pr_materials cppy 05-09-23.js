...
function AllPRMaterials() {
  ...
  PRMaterials.forEach((item, index) => {
    ...
    var text_field =
      "<div class='table-input'> <input type='text' min='1' max='" +
    ...
    item.indents.forEach((item_i, index1) => {
      ...
      var sub_checkbox =
        "<input type='checkbox' data-id='" +
        index1 +
        "' class='form-check-inputx rowCheckboxInside' id='select_" +
        index +
        "_" +
        index1 +
        "' />";
  ...
  $(document).on("click", "[type='checkbox']", function () {
    ...
    //console.log("desiredValue", idParts);
    if (idParts.length === 3 && idParts[0] === "select") {
      ...
      if ($(`#${main_id}`).prop("checked")) {
        var id = $(this).attr("data-id");
        var obj_length = PRMaterials[id].indents.length;
        console.log("PRMaterials obj length", obj_length);
      ...
      console.log("desiredValue", "index->", index, "subindex->", subindex);
      console.log(
        "============================================================================================="
      );
    }
  ...
  selected.forEach((item) => {
    item.indents.forEach((element) => {
      if (element.checkStatus == true) {
        if (element.remarks == "" || element.remarks == undefined) {
          check = false;
          return check;
        }
      }
